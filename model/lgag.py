...
class LGAG(nn.Module):
    def __init__(self, F_g, F_l, F_int, kernel_size=3, groups=1, activation='relu'):
        ...
        self.dtsa=DCTSA(freq_num=49, channel=F_int, reduction=1, groups=1, select_method='all')
        if kernel_size == 1:
            groups = 1
    ...
    def forward(self, g, x):
        g1 = self.W_g(g)
        x1 = self.W_x(x)
        psi = self.dtsa(self.activation(g1 + x1))
...
if __name__ == '__main__':
    x4=torch.randn(4,128,64,64).to('cuda')
    x6=torch.randn(4,128,64,64).to('cuda')
    model=LGAG(128,128,64,3,64).to('cuda')
    out=model(x4,x6)
    print(out.shape)